...
namespace Rivet {
  ...
  void getChildren(map<std::string,Histo1DPtr> _h, std::string name_template, Particle Parent, Particles& childrenList){
    if (debug){
        std::cout << "Getting children for candidate id = " << Parent.pid() << ", barcode = " << Parent.genParticle()->barcode() << ", status = " << Parent.genParticle()->status() << std::endl;
    }
    //check parent decay chain
    Particle Parent_to_decay = getLastInstance(Parent);
    for(const Particle &Child: Parent_to_decay.children()){
      if (debug) {
        std::cout << "Candidate child id = " << Child.pid() << ", child barcode = " << Child.genParticle()->barcode() << ", child status = " << Child.genParticle()->status() << std::endl;
      }
  ...
  public:
    ...
    void analyze(const Event& event) {
      vector<ConstGenParticlePtr> genParticles = HepMCUtils::particles(event.genEvent());
